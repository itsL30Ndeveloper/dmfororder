...
module.exports = async (message, client) => {
  ...
  if (message.author.bot || !message.content.startsWith(config.prefix)) return;
    ...
    if (!config.guild) return console.log("ID Guild belum Diberikan di Config.")
    
    let guild = client.guilds.cache.get(config.guild)
    
    if (!guild) return console.log("ID Guild yang tidak valid telah diberikan di Config.")
    
    if (!guild.me.hasPermission("ADMINISTRATOR")) return console.log("Bot Kehilangan Izin Administrator.\nIni hanya untuk memastikan bahwa bot tidak mengembalikan kesalahan apa pun karena kurangnya Izin.")
    
    if (!config.category) return console.log("Tidak ada ID Kategori yang Diberikan di Config.")
    
    let category = client.channels.cache.get(config.category)
    
    if (!category || category.type !== "category") return console.log("ID Kategori Tidak Valid telah Diberikan di Config.")
   ...
   let newticket = new discord.MessageEmbed()
     ...
     .addFields(
       ...
       { name: "Pembuatan akun", value: creation + " yang lalu", inline: false },
       { name: "Peran", value: guild.members.cache.get(message.author.id).roles.cache.map(r => r.toString()).join("\n"), inline: false },
       { name: "Catatan", value: "Gunakan perintah \`reply\` untuk membalas dan perintah \`close\` untuk menutup tiket.", inline: false }
       )
       
       ticket.send(newticket).then(m => m.pin()).catch(() => {})
     
     message.author.send("Orderan Anda telah Dibuat.\n Harap tunggu penjual untuk Merespons.\Anda dapat mengetik \`close\` jika Anda ingin menutup Tiket ini.").catch(() => {})
